...
const completed = document.getElementById('clear');
...
activeClass.forEach(tengo => {
  tengo.addEventListener('click', (e) => {
...
formIinputContainer.addEventListener('submit', (e) => {
  ...
  if (!value) {
    displayAlert('ERROR! EMPY SLOT.....', 'danger');
  }
  else {
    createTodoItem(id, value);
    displayAlert('SLOT ADDED...', 'success');
...
const RemoveCurrent = (e) => {
  ...
  displayAlert('SLOT DLETED!', 'success');
  removeFromLocalStorage(id);
  checkLeftItem();
  setBackToDefault();
